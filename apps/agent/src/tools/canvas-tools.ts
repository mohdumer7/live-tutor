...
export function createCanvasTools(
  ...
  return {
    ...
    generate_minigame: llm.tool({
      description:
        "Generate an interactive HTML minigame and embed it on the canvas in a sandboxed iframe. YOUR JOB is to pick the right rendering capability for the topic — the student should NEVER have to say 'make it 3D' or 'use physics'. Rules: (a) any physics topic (gravity, pendulums, projectiles, collisions, springs, planetary motion) → describe a 3D scene using three.js + cannon-es. (b) inherently 3D / spatial topics (geometry of solids, molecular structure, anatomy) → 3D scene. (c) 2D-only physics (billiards, lever, 2D pendulum if 3D is overkill) → matter.js. (d) drag/drop, matching, sorting, quizzes, sliders without physics → plain DOM. Be VERY specific in the description: rules, win condition, what the student does, what feedback to show, and explicitly name the library stack (e.g. 'use three.js + cannon-es, OrbitControls for camera'). Takes 5-20s (longer for 3D). Default size 420x360; use 500x400+ for 3D scenes. Returns a shape id.",
